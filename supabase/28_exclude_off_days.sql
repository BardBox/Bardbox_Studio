-- =============================================================
-- 28_exclude_off_days.sql
-- Skip task creation for excluded posting days:
--   • All Sundays
--   • 1st Saturday of the month  (day 1–7)
--   • 3rd Saturday of the month  (day 15–21)
-- =============================================================

-- Utility: returns true if the date is an excluded posting day
CREATE OR REPLACE FUNCTION public.is_excluded_posting_day(p_date date)
RETURNS boolean
LANGUAGE sql IMMUTABLE AS $$
  SELECT
    EXTRACT(DOW FROM p_date) = 0                                                        -- Sunday
    OR (EXTRACT(DOW FROM p_date) = 6 AND EXTRACT(DAY FROM p_date) BETWEEN 1  AND 7)   -- 1st Saturday
    OR (EXTRACT(DOW FROM p_date) = 6 AND EXTRACT(DAY FROM p_date) BETWEEN 15 AND 21); -- 3rd Saturday
$$;


-- Rebuild create_tasks_for_rows with the exclusion check + deadline floor
...
BEGIN
  FOREACH v_row.id IN ARRAY p_ids LOOP
    ...
    -- Skip off days (Sunday / 1st & 3rd Saturday)
    IF public.is_excluded_posting_day(v_row.posting_date) THEN
      CONTINUE;
    END IF;
